...
function Paddle({ hand }: any) {
  ...
  useFrame((state) => {
    values.current[0] = lerp(values.current[0], (hand.x * Math.PI) / 5, 0.2);
    values.current[1] = lerp(values.current[1], (hand.x * Math.PI) / 5, 0.2);
    api.position.set(hand.x * 10, (-hand.y - 0.5) * 5, hand.z);
...
export default function () {
  ...
  return (
    <>
      <Canvas shadows camera={{ fov: 50, position: [0, 5, 12] }}>
        ...
        >
          ...
          <Suspense fallback={null}>
            <Paddle pose={pose.left} />
            <Paddle pose={pose.right} />
          </Suspense>
        </Physics>
      </Canvas>
    </>
